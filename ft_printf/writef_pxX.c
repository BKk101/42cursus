...
/*   Updated: 2020/03/09 20:25:00 by bykim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int         write_fp(va_list ap, t_format f_info)
{
    ...
    temp = va_arg(ap, char *);
    str1 = apply_precision_num(convert_hex((long long)temp, 'x'), f_info); //precision 처리
    temp = ft_strjoin("0x", str1);
    size = (f_info.width > ft_strlen(temp) ? f_info.width : ft_strlen(temp)); //size 처리
    str2 = apply_minus_zero_width('p', size, temp, f_info); // -, 0, width 처리
    write(1, str2, size);
...
int         write_fxX(va_list ap, t_format f_info, char c)
{
    ...
    str1 = apply_precision_num(convert_hex(temp, c), f_info); //precision 처리
    size = (f_info.width > ft_strlen(str1) ? f_info.width : ft_strlen(str1)); //size 처리
    str2 = apply_minus_zero_width('x', size, str1, f_info); // -, 0, width 처리
    write(1, str2, size);
    ft_multifree(1, str2);
    return (size);
}