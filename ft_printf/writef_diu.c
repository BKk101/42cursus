...
/*   Updated: 2020/03/09 21:30:50 by bykim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int write_fdi(va_list ap, t_format f_info)
{
    ...
    num = va_arg(ap, int);
    str1 = apply_precision_num(ftt_itoa(num), f_info); //precision 처리
    ...
    str1 = apply_minus_zero_width('d', size, str2, f_info); // -, 0, width 처리
    write(1, str1, size);
    free(str1);
    return (size);
}
...
int write_fu(va_list ap, t_format f_info)
{
    ...
    num = va_arg(ap, int);
    str1 = apply_precision_num(ftt_uitoa(num), f_info); //precision 처리
    size = (f_info.width > ft_strlen(str1) ? f_info.width : ft_strlen(str1)); //size 처리
    str2 = apply_minus_zero_width('u', size, str1, f_info); // -, 0, width 처리
    write(1, str2, size);
    free(str2);
    return (size);
}